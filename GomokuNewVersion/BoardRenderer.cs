namespace GomokuNewVersion
{
    internal class BoardRenderer
    {
        ...
        public void Render(Board board, Position cursor, Stone turn, bool showCursor = true) // 보드 렌더러
        {
            Console.CursorVisible = false;
            RenderColumnLabels(cursor.Col, showCursor);

            // 보드 전체 칸 순회하여 보드판 출력
            for (int row = 0; row < Board.Size; row++)
            {
                RenderRowLabel(row, cursor.Row, showCursor);

                for (int col = 0; col < Board.Size; col++)
                {
                    Position cell = new Position(row, col);
                    Console.Write(GetCellSymbol(board, cell, cursor, turn, showCursor));
                }
